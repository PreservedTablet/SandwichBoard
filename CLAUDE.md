...
- NEVER attach an ad-platform MCP and scout/untrusted content in the same session. Draft/analyze sessions use mcp-draft.json only (`claude --mcp-config mcp-draft.json`); interactive campaign management uses mcp-manage.json. Never mix them.
...
- NEVER commit secrets, real account IDs, or operator-specific identifiers (personal names, Infisical workspace ids, emails). Roles are "maintainer" (repo decisions) and "operator" (deployments); deployment specifics live in Infisical and gitignored local files. Fixtures are synthetic (pnpm gen:fixtures, Phase 1+).
